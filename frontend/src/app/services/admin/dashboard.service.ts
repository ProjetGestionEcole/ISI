...
export class DashboardService {
  ...
  getStats(): Observable<DashboardStats> {
    ...
    return this.http.get<{data: DashboardStats}>(`${this.baseUrl}/my/dashboard-stats`).pipe(
      tap(response => {
  ...
  private transformApiResponse(apiData: DashboardStats): DashboardStats {
    return {
      ...apiData,
      // Transform API response to expected format
      totalEleves: apiData.etudiants_count || 0,
      totalEnseignants: apiData.enseignants_count || 0,
      totalClasses: apiData.classes_count || 0,
      totalMatieres: apiData.matieres_count || 0,
      // Add mock extended data for charts
      elevesByStatut: [
        { statut: 'inscrit', count: apiData.etudiants_count || 0, pourcentage: 87.3 },
        { statut: 'en_attente', count: Math.floor((apiData.etudiants_count || 0) * 0.1), pourcentage: 7.9 },
        { statut: 'suspendu', count: Math.floor((apiData.etudiants_count || 0) * 0.03), pourcentage: 3.6 },
        { statut: 'diplome', count: Math.floor((apiData.etudiants_count || 0) * 0.01), pourcentage: 1.2 }
      ],
      enseignantsByContrat: [
        { typeContrat: 'cdi', count: Math.floor((apiData.enseignants_count || 0) * 0.6), pourcentage: 59.8 },
        { typeContrat: 'cdd', count: Math.floor((apiData.enseignants_count || 0) * 0.3), pourcentage: 26.4 },
        { typeContrat: 'vacation', count: Math.floor((apiData.enseignants_count || 0) * 0.1), pourcentage: 13.8 }
      ],
      repartitionSpecialites: [
        { specialite: 'Informatique', codeSpecialite: 'INF', totalEleves: Math.floor((apiData.etudiants_count || 0) * 0.4), totalClasses: Math.floor((apiData.classes_count || 0) * 0.3), pourcentage: 40.0 },
        { specialite: 'Gestion', codeSpecialite: 'GES', totalEleves: Math.floor((apiData.etudiants_count || 0) * 0.3), totalClasses: Math.floor((apiData.classes_count || 0) * 0.3), pourcentage: 30.0 },
        { specialite: 'Commerce', codeSpecialite: 'COM', totalEleves: Math.floor((apiData.etudiants_count || 0) * 0.3), totalClasses: Math.floor((apiData.classes_count || 0) * 0.4), pourcentage: 30.0 }
      ]
    };
  }
